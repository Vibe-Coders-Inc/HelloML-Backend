...
async def create_text_document(
    ...
    try:
        ...
        # Verify ownership
        verify_agent_ownership(db, body.agent_id, current_user.id)
...
async def delete_document(
    ...
    try:
        ...
        # Get document with agent info
        existing = db.table("document").select("id, agent_id").eq("id", document_id).single().execute()
        if not existing.data:
            raise HTTPException(status_code=404, detail="Document not found")

        # Verify ownership through agent
        verify_agent_ownership(db, existing.data['agent_id'], current_user.id)
